...
Exports are available to all services in the consumer cluster. In the previous example, any service with the following permissions can read exports:
* `service:write` permissions for any service in the `frontend` partition.
* `service:read` and `node:read` for all services and nodes, respectively, in the `frontend` partition.
